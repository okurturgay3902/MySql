...
update people set name='adres daha sonra eklenecek' where address IS null;
...
update worker set worker_salary=(select max(worker_salary))*8/10 + worker_salary where worker_name="veli han";

-- Ali Can'ın maaşını en düşük maaştan yüzde 30 daha fazla düşürun
update worker  set worker_salary=(select min(worker_salary)*1.30  from (select * from worker)as w)
where worker_name='oli can';

-- Maaş ortalama maaştan düşükse maaşları 1000 artırın
update worker
set worker_salary=(worker_salary+1000) where worker_salary < (select avg(worker_salary)from (select * from worker) as a); 
