...
/*============================== JOIN İSLEMLERİ ===============================
    ...
    2) LEFT JOIN:  Ilk tabloda (Sol) olup sonuclari gosterir
    3) RIGHT JOIN: Ikinci(sag) tabloda olan tum sonuclari gosterir
...
------------------------------------------------------------------------------*/  
select sip.sirket_id, sir.sirket_isim, sip.siparis_id, sip.siparis_tarihi
from sirketler as sir inner join siparisler as sip
...
select sir.sirket_isim, sip.siparis_id, sip.siparis_tarihi, sir.sirket_id
from siparisler sip right join sirketler sir
on sir.sirket_id=sip.sirket_id;
...
------------------------------------------------------------------------------*/ 
 select p.personel_isim, b.bolum_isim from bolumler as b join personel as p
 on b.bolum_id=p.bolum_id where b.bolum_isim in('satis','muhasebe')
 order by b.bolum_isim, p.personel_isim;
 
 select p.personel_isim, b.bolum_isim from bolumler as b join personel as p
 on b.bolum_id=p.bolum_id where b.bolum_id in(10, 30)
 order by b.bolum_isim, p.personel_isim;
...
------------------------------------------------------------------------------*/
select p.personel_isim, b.bolum_isim from bolumler as b left join personel as p
 on b.bolum_id=p.bolum_id where b.bolum_isim in('satis','isletme', 'depo')
 order by p.personel_isim, b.bolum_isim; 
-- bolumler kismindan elemanlar istedigimiz icin left yazdik
...
select b.bolum_isim, p.personel_isim, p.maas from bolumler as b left join personel as p
...
select b.bolum_isim, p.personel_isim, p.maas from bolumler as b join personel as p
 on b.bolum_id=p.bolum_id where b.bolum_isim in('satis','mudurluk') and p.maas>2000
 order by b.bolum_isim, p.personel_isim;
...
------------------------------------------------------------------------------*/ 
 ...
 select p.mudur_id, b.bolum_isim, p.personel_isim, p.maas from bolumler as b right join personel as p
 on b.bolum_id=p.bolum_id where p.mudur_id in(1222, 1333);
...
select p.mudur_id, b.bolum_isim, p.personel_isim, p.maas,
 (select personel_isim from personel where p.mudur_id=personel_id) as mudur_isim
